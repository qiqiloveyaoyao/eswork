...
实验环境： kibana-8.12.2  elasticsearch-8.12.2  Windows11
  ...
  "aggs": {
    "product_count_per_category": {
      ...
      "aggs": {
        "product_count": {
          "cardinality": {
            "field": "product_id" // 使用cardinality聚合计算每个类别中不同产品的数量
          }
        }
      }
    }
...
结果 返回每返回每天的订单数量以及过去7天的移动平均订单数量天的订单数量以及过去7天的移动平均订单数量
      ...
      "aggs": {
        ...
        "this_month_vs_last_month": {
          "bucket_script": {
            ...
            "script": "(params.this_month - params.last_month) / params.last_month * 100"
          }
        }
      }
    }
